...
// よく使う並び替えをワンタップで適用（モーダルを開かずに使えるショートカット）
const sortPresets = [
  { label: "更新日：新しい順", levels: [{ key: "date", asc: false, group: false }] },
  { label: "名前：多い順",     levels: [{ key: "name", asc: true,  group: true  }] },
  { label: "⭐：高い順",       levels: [{ key: "fav", asc: false, group: false }] },
  { label: "自分評価：高い順", levels: [{ key: "selfRating", asc: false, group: false }] },
  { label: "No：登録順",       levels: [{ key: "no", asc: true, group: false }] },
];

window.applySortPreset = (i) => {
  const preset = sortPresets[i];
  if (!preset) return;
  sortLevels = preset.levels.map(l => ({ ...l }));
  saveSortLevels();
  closeSortModal();
  render();
  showToast(`「${preset.label}」で並び替えました`, "success");
};
